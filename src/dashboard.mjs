...
import { hostedWrappedUrl, hostedSubmitUrl } from "./hosted.mjs";
...
// string to show it, omit or pass `false` to hide it (local-only). submitUrl is
// the "get on the board" button (the email-verified leaderboard entry) and rides
// with hostedUrl. ALL render as an <a href> a human clicks — the page still
// auto-requests nothing.
//
// Every payload is STAMPED with app_version. The stamp is what makes a rendered
// report identifiable after the fact: a report.html on disk carrying no stamp (or
// an older one) was generated by an older build — e.g. before the censor spans
// existed — and is therefore known-stale. Reports are rewritten from the ledger on
// every `init` / `dashboard` run, so a stale file is replaced the next time the
// user runs either command.
export function renderDashboard(stats, opts = {}) {
  const { donateUrl, hostedUrl, submitUrl } = opts;
  ...
  const payload = { ...stats, app_version: APP_VERSION };
  ...
  if (typeof submitUrl === "string" && submitUrl) payload.submit_url = submitUrl;
...
export function writeDashboard(records, opts = {}) {
  ...
  // The board link rides WITH the lights button: it is the same hosted road
  // carrying the same aggregate numbers, so whenever the lights are hidden
  // (--local / SWEAR_JAR_LOCAL_ONLY / an empty ledger) a hosted URL is never put
  // in front of the user, and the board button hides with it.
  let submitUrl = opts.submitUrl;
  if (submitUrl === undefined) {
    if (!hostedUrl) {
      submitUrl = false;
    } else {
      try {
        submitUrl = hostedSubmitUrl(stats);
      } catch {
        submitUrl = false; // same rule: a URL hiccup never blocks the local report
      }
    }
  }
  const html = renderDashboard(stats, { ...opts, hostedUrl, submitUrl });
