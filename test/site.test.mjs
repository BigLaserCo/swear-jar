...
function readSiteCss() {
  return fs.readFileSync(path.join(DOCS, "site.css"), "utf8");
}
...
test("landing keeps the scanner CTA inside the Claude-inspired visual language", () => {
  const html = readPage("index.html");
  const css = readSiteCss();

  assert.match(html, /class=["']wrap landing["']/, "landing is explicitly scoped from shared pages");
  assert.match(html, /class=["']hero landing-hero["']/, "hero receives the editorial landing treatment");
  assert.match(html, /class=["']report-card landing-comparison["']/, "sample comparison is a distinct landing panel");
  assert.match(html, /class=["']meter sample-swear["']/i, "sample swear count has an explicit color lane");
  assert.match(html, /class=["']meter sample-kind["']/i, "sample kindness count has an explicit color lane");
  assert.match(html, /class=["']sample-verdict["']/, "sample verdict uses the stamped treatment");

  for (const token of ["#0f1216", "#272d37", "#e8e6e1", "#e8853a", "#5fb07a", "#8a93a0"]) {
    assert.ok(css.includes(token), `landing stylesheet includes ${token}`);
  }
  assert.match(css, /\.landing\s+\.landing-hero/, "editorial hero styles are landing-scoped");
  assert.match(css, /\.landing\s+\.sample-swear\s+i\{[^}]*#e8853a/, "swear sample meter is orange");
  assert.match(css, /\.landing\s+\.sample-kind\s+i\{[^}]*#5fb07a/, "kindness sample meter is green");
  assert.match(css, /\.landing\s+\.sample-verdict\{[^}]*var\(--landing-kind\)/, "verdict gets a green stamp treatment");
  assert.match(css, /\.landing\s+\.step\{min-width:0\}/, "mobile command cards may shrink without creating horizontal overflow");
});
